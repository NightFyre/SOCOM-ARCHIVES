...
public:
	template <size_t... Is>
	constexpr __forceinline kjasdjw(const char* str, std::index_sequence< Is... >) : _key(RandomChar< K >::value), _encrypted { enc(str[Is])... }
	{
...
#if RELEASE
#define crypt_str(s) kjasdjw <sizeof(s) - 1, __COUNTER__> (s, std::make_index_sequence <sizeof(s) - 1>()).decrypt()
#else
