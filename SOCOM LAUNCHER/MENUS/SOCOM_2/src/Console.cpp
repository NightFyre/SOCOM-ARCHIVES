...
namespace SOCOM2 {
	...
	void Console::InitializeConsole(const char* ConsoleName)
	{
		...
		//	INIT INFO
		g_Console->printdbg("[+] SOCOM 2 MENU (PREVIEW)\n", g_Console->color.yellow);
		g_Console->printdbg("[+] BUILD VERSION: v1.1\n", g_Console->color.yellow);
		g_Console->printdbg("[+] BUILD DATE: 6/24/22\n", g_Console->color.yellow);
		g_Console->printdbg("[+] Created by NightFyre\n\n", g_Console->color.DEFAULT);
		...
		g_Console->printdbg("PLEASE LAUNCH SOCOM 2 & TOGGLE FULLSCREEN\n\n", g_Console->color.DEFAULT);
		return;
	}
