...
namespace SOCOM2
{
	...
	template<typename T> inline T PS2Read(uintptr_t Addr)
	{
		uintptr_t PS2Addr = g_GameFunctions->GetPS2Addr(Addr);
		T A{};
		A = *(T*)PS2Addr;
		return A;
	}

	template<typename T> inline void PS2Patch(uintptr_t Addr, T Patch)
	{
		uintptr_t PS2Addr = g_GameFunctions->GetPS2Addr(Addr);
		*(T*)PS2Addr = Patch;
		return;
