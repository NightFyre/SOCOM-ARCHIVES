...
namespace SOCOM1 {
	...
	void Console::InitializeConsole(const char* ConsoleName)
	{
		...
		printdbg("[+] SOCOM INTERNAL (PREVIEW)\n", g_Console->color.yellow);
		printdbg("[+] BUILD VERSION: v1.0\n", g_Console->color.yellow);
		printdbg("[+] BUILD DATE: 6/16/22\n", g_Console->color.yellow);
		printdbg("[+] Created bv NightFyre\n\n", g_Console->color.yellow);
		printdbg("[!] THIS IS ONLY FOR SOCOM 1 (black edition)\n", g_Console->color.red);
		printdbg("[!] THIS IS A PREVIEW BUILD\n", g_Console->color.red);
		printdbg("[!] PLEASE DON'T INJECT UNTIL YOU HAVE LAUNCHED THE GAME [INSERT]\n\n", g_Console->color.red);
		printdbg("PCSX2 LAUNCHED\n", g_Console->color.green);
		printdbg("PLEASE LAUNCH SOCOM COMBINED ASSAULT & TOGGLE FULLSCREEN\n\n", g_Console->color.DEFAULT);
		return;
	}
